/* jshint -W104 *//*jshint -W080 *//*jshint -W119 */
const PFTC_PACKAGE_NAME = "pathfoundry";
const PFTC_SYSTEM_IDENTIFIER = 'd20pf_ftc';

/* ------------------------------------------- */
/* PFTC GLOBALS AND UTILITY FUNCTIONS            */
/* ------------------------------------------- */
const PFTC = {

    ROOT_DIR: 'workshop/'+PFTC_PACKAGE_NAME+'/',
    TEMPLATE_DIR: 'workshop/'+PFTC_PACKAGE_NAME+'/html/',
    CSS_DIR: 'workshop/'+PFTC_PACKAGE_NAME+'/css/',

    /* ------------------------------------------- */

    init: function() {
        hook.call("PFTCInit");
        $('body').append('<link rel="stylesheet" href="'+ PFTC.CSS_DIR + 'ftc.css" type="text/css" />');
        console.log("Pathfinder Foundry | Mod enabled and loaded!");
    },
    ...
    loadTemplate: function(path) {
        ...
        */
        ...
        let message = `Pathfinder Foundry | Stored template ${path} to cache.`;
        console.log(message);
        return html;
    },
    ...
    populateTemplate: function(html, data) {
        ...
        */
        ...
        /**
         * Objective:
         *   If the data indicates a HeroLab-linked character
         *   just return the html unchanged, otherwise,
         *   carry on as usual.
         */

        return html.replace(/{% checked-if ([\w.]+)%}/g, function(match, attr) {
            var val = PFTC.getProperty(data, attr);
    ...
    setProperty: function(data, name, value, dtype) {
        ...
        value = PFTC.cleanValue(value, dtype);

        // Get the data target
        let [target, key] = PFTC.getTargetKey(data, name);
...
class PFTCEntity {
    ...
    getData(name) {
        PFTC.getProperty(this.data, name);
    }

    setData(name, value, dtype) {
        PFTC.setProperty(this.data, name, value, dtype);
        this._changed = true;
    }
    ...
    buildHTML(data, scope) {
        throw "Pathfinder Foundry | A PFTCEntity subclass must implement the buildHTML method.";
    }

    /* ------------------------------------------- */

    populateHTML(html, data) {
        return PFTC.populateTemplate(html, data);
    }
...
hook.add("Initialize", "PFTCSetup", function(...args) {
    let gameid = game.templates.identifier;

    // Only initialize PFTC if we are using the correct system OR no system at all
    if ( gameid === PFTC_SYSTEM_IDENTIFIER ) PFTC.init();
    else console.log("Pathfinder Foundry | Mod enabled but not active for system " + gameid);
});
