...
class PFTCElement extends PFTCEntity {
  ...
  /* Templates */
  get mainTemplate() {
    const td = PFTC.TEMPLATE_DIR + "elements/";
  ...
  static applyDataModel() {
    ...
    $.each(game.templates.elements, function (type, definition) {
      ...
      mergeObject(definition, PFTC.elements.core, true, true, true);
      ...
      if (isInventory.includes(type)) mergeObject(definition, PFTC.elements.inventory, true, false, true);

      // Add type-specific data
      mergeObject(definition, PFTC.elements[type], true, false, true);
    });
    console.log("Pathfinder Foundry | Element Templates Updated");
  }
  ...
  buildHTML(data, scope) {
    let html = PFTC.loadTemplate(this.mainTemplate);
  ...
  static editOwnedItem(owner, container, data, itemId) {
    ...
    const app = sync.newApp("PFTC_RENDER_ELEMENT", obj, scope);
  ...
  static renderChatHTML(chatData) {
    ...
    let html = PFTC.loadTemplate("html/actions/item.html");
    html = $(PFTC.populateTemplate(html, chatData));
    return html;
  }
...
class PFTCWeapon extends PFTCElement {
  ...
  getChatData() {
    ...
    data.props = PFTC.ui.chatProperties(props);
    return data;
  }
  ...
  static renderChatHTML(chatData) {
    ...
    // Load and populate the chat template
    let html = PFTC.loadTemplate("html/actions/weapon.html");
    html = $(PFTC.populateTemplate(html, chatData));

    // Weapon Attack
    html.find("h3.action-roll.weapon-hit").click(function () {
      PFTCItemActions.weaponAttack($(this));
    });

    // Weapon Damage
    html.find("h3.action-roll.weapon-damage").click(function () {
      PFTCItemActions.weaponDamage($(this));
    });
    return html;
  }
}
...
class PFTCSpell extends PFTCElement {
  ...
  _scaleCantripDamage(formula, level) {
    ...
    return PFTC.Dice.alter(formula, 1, undefined, multi);
  }
  ...
  static renderChatHTML(chatData) {
    ...
    // Load and populate the chat template
    let html = PFTC.loadTemplate("html/actions/spell.html");
    html = $(PFTC.populateTemplate(html, chatData));

    // Spell Attack
    html.find("h3.action-roll.spell-hit").click(function () {
      PFTCItemActions.spellAttack($(this));
    });

    // Spell Damage
    html.find("h3.action-roll.spell-damage").click(function () {
      PFTCItemActions.spellDamage($(this));
    });
    return html;
  }
}
...
class PFTCFeat extends PFTCElement {
...
class PFTCTool extends PFTCElement {
  ...
  chatAction() {
    ...
    PFTC.ui.createDialogue(html, {
      ...
      close: function () {
        ...
        PFTC.Dice.roll(actor, flavor, formula, {"mod": mod, "prof": prof});
      }
    });
  };
...
hook.add("PFTCInit", "Elements", function() {

  // Apply the Item Data Model
  PFTCElement.applyDataModel();

  // Render Item Sheets
  sync.render("PFTC_RENDER_ELEMENT", function (obj, app, scope) {
    const element = PFTCElement.fromData(obj, scope);
    return element.render(app, scope);
  });

  // Register Item Character Drop Hook
  hook.add("OnDropCharacter", "PFTCOnDrop", function (obj, app, scope, dt) {
    ...
    if (item instanceof PFTCSpell && owner.data.tabs["content-tabs"] === "tab-inventory") {
      item = item.toScroll();
    }
